...
class Dockpallet(Node):
    ...
    def __init__(self):
        ...
        self.switch_state_pub = self.create_publisher(String, '/limit_switch_state', 10)
        ...
        self.dock_state_switch = 'False'
        
        try:
            self.arduino_nano = serial.Serial(port=self.port, baudrate=self.baudrate, timeout=0)
        ...
        self.create_timer(0.1, self.read_switch)
        ...
        if self.dock_flag and self.navigate_flag :
            ...
            try:
                ...
                if abs(distance) > 0.2:
                    ...
                    # self.switch_state.data = str(self.dock_state_switch)
                    # self.switch_state_pub.publish(self.switch_state)
                    
                    self.diagnostics.data = "Docking Under Process..."
                    self.docking_undocking_diagnostics.publish(self.diagnostics)
                    
                    if self.dock_state_switch == 'The switch: is Pressed':
        ...
        if self.undock_flag and self.navigate_flag :
            ...
            try:
                ...
                if abs(distance) < 0.7:
                    ...
                    if self.dock_state_switch == 'The switch: Is released':
                        self.switch_state.data = "Undocking Successful ... "
                        self.switch_state_pub.publish(self.switch_state)
                        self.get_logger().warn('Undocking SCCESSFUL')
                    
                    else:
                        self.switch_state.data = "Undocking unsuccessful...."
                        self.switch_state_pub.publish(self.switch_state)
                        self.get_logger().warn('Undocking UNSCCESSFUL')
    ...
    def read_switch(self):
        try:
            switch_state = self.arduino_nano.readline().decode().strip()
